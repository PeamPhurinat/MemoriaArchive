...
const ReviewPage = ({ project, setProject }) => {
  ...
  return (
    <div className="ma-review-page">
      <div className="app-shell ma-review-shell">
        <h1 className="page-title">Review Project</h1>
        <p className="page-subtitle ma-review-subtitle">
          Review & Organize (Required): check everything before entering 3D room.
        </p>
        <div className="ma-review-meta-row">
          <span className="ma-review-meta-pill">{memorySlotCount} total slots</span>
          <span className="ma-review-meta-pill">
            {canGenerate3D ? "Ready for 3D generation" : "Add at least one slot to continue"}
          </span>
        </div>

        <div className="section ma-review-section ma-review-project-summary">
          <h2>{project.title || 'Untitled Project'}</h2>
          <p className="helper">{project.coupleNames || 'No couple names added yet.'}</p>
          {project.reviewApprovedAt ? (
            <p className="helper">Last approved: {new Date(project.reviewApprovedAt).toLocaleString('en-US')}</p>
          ) : null}
        </div>

        <div className="section ma-review-section">
          <h2>Photos</h2>
          {allPhotos.length === 0 ? (
                ...
                <div className="list-item" key={`${photo.source}-${photo.id}-${index}`}>
                  ...
                  <div className="photo-thumb" style={{ width: '120px' }}>
                    {photo.hasPhoto ? (
                      ...
                      <div className="small-note">No image</div>
                    )}
                  </div>
                  <div>
                    {photo.name}
        ...
        <div className="button-row ma-review-actions">
          <button className="button ma-review-action-btn" type="button" onClick={() => navigate('/project-detail')}>
            Back to Edit
          </button>
          <button
            className="button primary ma-review-action-btn"
            type="button"
        ...
        {!canGenerate3D ? (
          <p className="error-text" style={{ marginTop: '12px' }}>
            Add at least 1 memory slot before generating 3D room.
          </p>
        ) : null}
      </div>
    </div>
